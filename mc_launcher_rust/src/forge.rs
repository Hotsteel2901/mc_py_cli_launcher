...
impl ForgeManager {
    ...
    pub fn get_available_versions(&self, mc_version: &str) -> Vec<String> {
        let (status, body) = http::http_get(FORGE_MAVEN_META).unwrap_or_else(|e| {
            crate::die!(format!("Forge maven metadata fetch failed: {}", e));
        });
        if status != 200 {
            crate::die!(format!("Forge maven metadata fetch failed ({})", status));
        }

        let text = String::from_utf8_lossy(&body);
        let prefix = format!("{}-", mc_version);

        // Simple XML parse using regex (matches version tags)
        let version_re =
    ...
    pub fn install(
        ...
        let java_path = java::check_java(None).unwrap_or_else(|| {
            crate::die!("Java not found. Cannot run Forge installer.");
        });
        ...
        let status = Command::new(&java_path)
            ...
            .status()
            .unwrap_or_else(|e| {
                crate::die!(format!("Failed to run Forge installer: {}", e));
            });
        ...
        if !version_json_path.exists() {
            crate::die!(
                "Forge installer finished but version.json was not found."
            );
        }
    ...
    fn build_profile(
        ...
        let version_data: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(version_json_path).unwrap())
                .unwrap();
    ...
    fn resolve_inherits(
        ...
        let parent: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&parent_path).unwrap())
                .unwrap();
